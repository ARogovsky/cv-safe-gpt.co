...
import { getAltPaths, getPageTitles, getSectionLabels, getUkSlugs } from './articles/registry'
 ...
 * - Inner pages: permanent "← esupport.org.ua" back link
...
function useLang() {
  ...
  const lang: 'uk' | 'en' = UK_SLUGS.has(pathname) ? 'uk' : 'en'
...
function useLanguageBanner(lang: Lang) {
  ...
  useEffect(() => {
    ...
    const browserPrefersUk = navigator.language.toLowerCase().startsWith('uk')
    const mismatch = (lang === 'uk' && !browserPrefersUk) || (lang === 'en' && browserPrefersUk)
    if (!mismatch) return
...
/** Circular flag icons — Ukraine (blue-yellow) and UK (Union Jack simplified) */
function FlagUK({ className = "w-4 h-4" }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 16 16" aria-hidden="true">
      <clipPath id="flagCircleUK"><circle cx="8" cy="8" r="8" /></clipPath>
      <g clipPath="url(#flagCircleUK)">
        <rect y="0" width="16" height="8" fill="#0057B7" />
        <rect y="8" width="16" height="8" fill="#FFD700" />
      </g>
    </svg>
  )
}
...
export default function GlobalNav() {
  ...
  const bannerMessage = showBanner ? (
    ...
    >
      ...
      >
        {t.ui.languageBannerSwitchPrefix}{lang === 'uk' ? <FlagEN className="w-3.5 h-3.5 mx-0.5" /> : <FlagUK className="w-3.5 h-3.5 mx-0.5" />}{t.ui.languageBannerSwitchLang}
      </button>
      <button
        onClick={dismiss}
            ...
            {!isHome && (
              ...
              >
                ...
                >
                  ...
                  <span className="hidden sm:inline">esupport.org.ua</span>
                </Link>
                {pageTitle && (
                  <>
